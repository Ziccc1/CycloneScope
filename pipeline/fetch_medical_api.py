...
for geom in zones.geometry:
    ...
    payload = requests.get(url, timeout=60).json().get("value", [])
    for item in payload:
...
qa = {"count": len(gdf), "duplicate_id_count": int(gdf.facility_id.duplicated().sum()), "bounds": [float(x) for x in gdf.total_bounds], "crs": "EPSG:4326", "query_radius_m": args.radius}
