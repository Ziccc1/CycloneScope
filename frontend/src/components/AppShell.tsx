...
const modes: { value: AnalysisMode; label: string }[] = [
  ...
  { value: 'taiwan-scenario', label: '台湾情景 · 5场' },
]
...
export default function AppShell({
  ...
  const taiwanStorms = useMemo(() => storms.filter((storm) => storm.impact_available), [storms])
  const visibleStorms = state.mode === 'taiwan-scenario' ? taiwanStorms : storms
  const compared = [
    selected,
  ...
  useEffect(() => {
    if (state.mode !== 'taiwan-scenario' || selected?.impact_available || taiwanStorms.length === 0) return
    const preferred = taiwanStorms.find((storm) => storm.name.toUpperCase() === 'MORAKOT') ?? taiwanStorms[0]
    dispatch({ type: 'select-taiwan-storm', stormId: preferred.id })
  }, [dispatch, selected?.impact_available, state.mode, taiwanStorms])
      ...
      <div className="workspace">
        <aside className="left-panel panel">
          ...
          <section className="case-library">
            <div className="panel-heading">
              <div>
                <p className="eyebrow">{state.mode === 'taiwan-scenario' ? 'TAIWAN-READY EVENTS' : 'EVENT LIBRARY'}</p>
                <h2>{state.mode === 'taiwan-scenario' ? '台湾分析案例' : '高影响案例'}</h2>
              </div>
              <span className="count">{visibleStorms.length}</span>
            </div>
            {state.mode === 'taiwan-scenario' && (
              <p className="taiwan-case-note">
                以下 5 场具备台湾 ERA5 影响格网，可直接切换并进行设施配置分析。
              </p>
            )}
            {visibleStorms.length === 0 && !loading && (
              <p className="empty">
                {state.mode === 'taiwan-scenario' ? '当前数据包没有可用的台湾影响案例。' : '当前筛选没有案例。'}
              </p>
            )}
            {visibleStorms.map((storm) => (
              <article
                key={storm.id}
                className={`case-item ${state.selectedStormId === storm.id ? 'selected' : ''} ${state.mode === 'taiwan-scenario' ? 'taiwan-case-item' : ''}`}
              >
                <button
                  ...
                  onClick={() => {
                    ...
                    dispatch(
                      state.mode === 'taiwan-scenario'
                        ? { type: 'select-taiwan-storm', stormId: storm.id }
                        : { type: 'select-storm', stormId: storm.id },
                    )
                  }}
                >
                  <span className="case-meta">
                    <span>{storm.basin} · {storm.season}</span>
                    {storm.impact_available && <b className="taiwan-ready-badge">台湾分析</b>}
                  </span>
