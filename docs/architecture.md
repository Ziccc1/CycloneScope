...
本文后续的架构草图和初始实现边界属于设计基线。当前可交付数据状态以 docs/data-processing/DATA-PROCESSING-HANDOFF-v2.1.md 为准：真实数据处理、契约化台湾设施（含 787 条救援设施）、13 个 ERA5 manifest/665 个 frame 和 5 个影响格网已经生成。ERA5 实际时间间隔为 6 小时；前端必须按能力矩阵区分动态、静态和无风场。
...
3. 经典气旋真实风场：ERA5 区域裁剪；时间间隔以实际 manifest 为准（当前为 6 小时）；
...
以上描述是初始骨架快照。当前真实数据处理和契约交付已完成；前端 MapLibre、粒子风场和空间评估仍由 B/C 集成，能力以 v2.1 交付文档为准。
